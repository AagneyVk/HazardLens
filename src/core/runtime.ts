import type { SimEvent, Twin, TwinContext, WorldSnapshot } from "./types.js";
...
export class SimulationRuntime {
 ...
 step(dt:number){if(!Number.isFinite(dt)||dt<=0)throw new Error("dt must be positive");this.drainEvents();const c=this.context();for(const t of [...this.registry.values()])if(t.state.active)t.tick(dt,c);this.time+=dt;this.drainEvents()}
 run(duration:number,dt=.25){if(!Number.isFinite(duration)||duration<0||!Number.isFinite(dt)||dt<=0)throw new Error("Invalid simulation duration or timestep");const end=this.time+duration;while(this.time+1e-9<end)this.step(Math.min(dt,end-this.time))}
 ...
 private materialize(e:SimEvent){if(e.type==="release.created"){const p=e.payload as any,id=`release-${e.id}`;if(!this.registry.has(id))this.add(new ReleaseTwin(id,p.origin,e.sourceId,Number(p.rateKgS)))}if(e.type==="fire.created"){const p=e.payload as any,id=`fire-${e.id}`;if(!this.registry.has(id))this.add(new FireTwin(id,p.origin,Number(p.intensityMw)))}}
