...
const cameraTarget={p:new THREE.Vector3(155,125,185),t:new THREE.Vector3(0,0,-35)};let cinematic=false;
const command=new CommandCenter({rupture:()=>sim.breakPipe(),isolate:()=>sim.isolatePipe(),suppress:()=>sim.suppress(),reset:()=>{sim.reset();inspector.show()},overview:()=>{cameraTarget.p.set(155,125,185);cameraTarget.t.set(0,0,-35);cinematic=true},incident:()=>{cameraTarget.p.set(-48,30,-65);cameraTarget.t.set(-80,1,-112);cinematic=true}});
renderer.domElement.addEventListener('pointerdown',e=>{if((e.target as HTMLElement).tagName!=='CANVAS')return;pointer.x=e.clientX/innerWidth*2-1;pointer.y=-(e.clientY/innerHeight)*2+1;const snap=sim.snapshot();const selected=world.pick(raycaster,camera,pointer,snap);inspector.show(selected)});
let last=performance.now();function frame(now:number){requestAnimationFrame(frame);const dt=Math.min(.05,(now-last)/1000);last=now;sim.update(dt);const snap=sim.snapshot();world.sync(snap,dt);command.update(snap);if(cinematic){camera.position.lerp(cameraTarget.p,Math.min(1,dt*2.4));controls.target.lerp(cameraTarget.t,Math.min(1,dt*2.4));if(camera.position.distanceTo(cameraTarget.p)<.5)cinematic=false}controls.update();renderer.render(scene,camera)}requestAnimationFrame(frame);
